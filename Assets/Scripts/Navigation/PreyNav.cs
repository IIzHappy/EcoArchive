...
public class PreyNav : MonoBehaviour
{
    ...
    private void Move()
    {
      for(int i = 0; i < maxSampleAttempts; i++)
        {
            Vector3 randDirection = Random.insideUnitSphere * sampleRadius;
            randDirection += transform.position;
            randDirection.y = 0f;

            if(NavMesh.SamplePosition(randDirection, out NavMeshHit hit, sampleRadius, NavMesh.AllAreas))
            {
